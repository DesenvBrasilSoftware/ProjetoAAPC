...
<form class="needs-validation" novalidate id="form" action="/grupoItem.store" method="post">
  @csrf
  <input type="hidden" id="id" name="id" value="{{ $grupo_item->id }}">
  <div class="form-group">
    <label for="descricao">Descrição:</label>
    <input required type="text" name="descricao" class="form-control" id="descricao" maxlength="45"
      value="{{ $grupo_item->descricao }}" autofocus placeholder="Digite a descrição do grupo do item">
  </div>
  <div class="form-group">
  ...
  <div class="form-group">
    ...
    data-off="Não" {{ $grupo_item->refrigerado ? 'checked' : '' }}>
  </div>
  <div class="form-group">
